...
- **`migrations.js`** — schema versioning for saved records (session 2026-07-10). `SCHEMA_VERSION` (currently **1**) + `migratePatient(p)` / `migrateAll(map)` upgrade any old-shaped record to the current shape in ONE place (idempotent, additive/defensive — fills missing `visits`/`images`/`alerts`/`sections` arrays + ids, never clobbers real data; does NOT normalise photos since string|{path} are both valid). Applied in `getPatients()` (migrate-on-read), once at startup via `migrateStored()` (persists the upgrade), and in `cloudPull` (upgrades pulled remote records). Same dual browser/Node pattern; cached in `sw.js` (v9). **To add a migration:** raise `SCHEMA_VERSION`, add an `if(v<N){…;v=N;}` block, add a test.
- **Tests/CI:** `scoring.test.js` + `interventions.test.js` + `migrations.test.js` (Node built-in `node --test`, zero deps) pin score bands, escalation thresholds, safety-critical intervention rules, and migration invariants. `.github/workflows/test.yml` runs them on every push/PR; `package.json` has `"test":"node --test"`. Run locally: `node --test` (**53 tests**). **When you change a threshold, safety rule, or record shape, update its test.**
...
- **Wound photos → Supabase Storage (session 2026-07-10):** private bucket `wound-photos`, photos at path `<mrn>/<visitId>/<idx>.jpg`, shown via short-lived **signed URLs**. A visit image entry is EITHER a legacy inline data-URL **string** OR a `{path}` **object** — both render (backward compatible). Flow: `saveToLog` still saves photos **inline first** (reliable, unchanged), then `offloadVisitPhotos(mrn,vid)` (best-effort, async) uploads them, rewrites entries to `{path}`, and re-saves — slimming localStorage AND the synced blob. **If Storage isn't set up / offline / any failure → the photo just stays inline (old behaviour), nothing lost** — so it was safe to ship before the bucket existed. Key fns in `index.html`: `offloadVisitPhotos`, `deleteCloudPhotos` (cleanup on delete), `photoImg(entry)` (render helper), `resolvePhotoUrls(scope)` (fills signed URLs, cached ~55 min in `_signedUrls`), `dataUrlToBlob`, `photoStorageReady`. CSP `img-src` includes `*.supabase.co`. **User must run `storage-setup.sql` once** to create the bucket + RLS (until then, inline fallback). Chosen model: new photos only (existing inline photos are left as-is).
...
**Android APK — BUILT (session 2026-07-11, via PWABuilder Google Play package)**
- The user built it in PWABuilder. It generated its OWN identity (did not use the earlier prepared keystore): **package `io.github.hussein_zreik.twa`**, keystore alias `my-key-alias`, store/key password `gwbrKyfF1B6J`, **SHA-256 `EB:93:22:85:…:16:97`** (verified via keytool).
- `.well-known/assetlinks.json` was **updated to these real values** and deployed, so the installed TWA verifies and opens full-screen (no URL bar). The old `app.curanova.twa` / `55:24:3D:…` values and `curanova-signing.keystore` are now **obsolete** for this app.
- Package (in the uploaded zip) contains: `CuraNova.apk` (sideload — share with team), `CuraNova.aab` (Google Play only, can't sideload), `signing.keystore` + `signing-key-info.txt` (**the ONLY key that can publish updates now — user must keep it safe**), and its own `assetlinks.json`.
- **Play Store note:** uploading the `.aab` to Google Play triggers Play App Signing (Google re-signs), so the on-device fingerprint becomes Google's — assetlinks would then need Google's SHA-256 (Play Console → App Integrity → App signing key cert) added as a second fingerprint. Not needed for direct APK sharing.
- **Manifest is store-complete** (session 2026-07-10): `id`, `lang`, `dir`, `categories`, `launch_handler`, split icons (`any`+`maskable`), 3 phone `screenshots` (`screenshot-1/2/3.png`, 824×1830). Optional polish still open: maskable icon has little safe-zone padding (mark tips may clip).
...
5. **Enable wound-photo cloud storage** — run `storage-setup.sql` once in Supabase → SQL Editor (creates the private `wound-photos` bucket + access rules). Until then the app keeps photos inline (works, but the localStorage cap and full-blob re-upload persist). After running it, new photos automatically move to Storage.
